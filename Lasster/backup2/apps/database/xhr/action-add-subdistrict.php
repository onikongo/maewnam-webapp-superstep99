<?php
	session_start();
	include_once "../../../config/define.php";
	include_once "../../../include/db.php";
	include_once "../../../include/oceanos.php";
	
	@ini_set('display_errors',DEBUG_MODE?1:0);
	date_default_timezone_set(DEFAULT_TIMEZONE);
	
	$dbc = new dbc;
	$dbc->Connect();
	$os = new oceanos($dbc);
	
	if($dbc->HasRecord("subdistricts","name = '".$_REQUEST['txtName']."'")){
		echo json_encode(array(
			'success'=>false,
			'msg'=>'subdistrict Name is already exist.'
		));
	}else{
		$data = array(
			'#id' => "DEFAULT",
			'name' => $_REQUEST['txtName'],
			'#district' => $_REQUEST['txtDistrict'],
			'#city' => $_REQUEST['txtCity'],
			'#country' => $_REQUEST['txtCountry']
		);
		
		if($dbc->Insert("subdistricts",$data)){
			$subdistrict_id = $dbc->GetID();
			echo json_encode(array(
				'success'=>true,
				'msg'=> $subdistrict_id
			));
			
			$subdistrict = $dbc->GetRecord("subdistricts","*","id=".$subdistrict_id);
			$os->save_log(0,$_SESSION['auth']['user_id'],"subdistrict-add",$subdistrict_id,array("subdistricts" => $subdistrict));
		}else{
			echo json_encode(array(
				'success'=>false,
				'msg' => "Insert Error"
			));
		}
	}
	
	$dbc->Close();
?>