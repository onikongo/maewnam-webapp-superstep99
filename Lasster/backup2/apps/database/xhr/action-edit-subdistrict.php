<?php
	...
	if($dbc->HasRecord("subdistricts","name = '".$_REQUEST['txtName']."' AND id != ".$_REQUEST['txtID'])){
		...
		if($dbc->Update("subdistricts",$data,"id=".$_REQUEST['txtID'])){
			...
			$subdistrict = $dbc->GetRecord("subdistricts","*","id=".$_REQUEST['txtID']);
			$os->save_log(0,$_SESSION['auth']['user_id'],"subdistrict-edit",$_REQUEST['txtID'],array("subdistricts" => $subdistrict));
		}else{
			echo json_encode(array(
				'success'=>false,
				'msg' => "No Change"
