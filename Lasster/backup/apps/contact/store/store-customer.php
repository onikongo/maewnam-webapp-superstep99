<?php
	...
	$aColumns = array(
		...
		"IF(customers.status=0,'Inactive','Active')",
		"IF(customers.last_login IS NULL,'-',customers.last_login)",
		'customers.id'
	);
	$sIndexColumn = "customers.id";
	
	$sTable = "customers
		LEFT JOIN customer_groups ON customers.gid = customer_groups.id
		LEFT JOIN contacts ON customers.contact = contacts.id
	";
	$sLimit = "";
