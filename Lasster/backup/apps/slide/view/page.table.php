...
$(function(){
	...
	$("#tbl").DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "apps/slide/store/store.php",
		"aoColumns": [
		...
		"createdRow": function ( row, data, index ) {
			...
			s= '';
			s += '<img  width="100%" src="'+data[2]+'">';
			$('td', row).eq(2).html(s);
			s = '';
			if(data[3]==1){
				s +='<div class="switch">';
				s +='<input id="cmn-toggle-'+data[0]+'" class="cmn-toggle cmn-toggle-round" checked type="checkbox" onClick="fn.app.slide.active('+data[0]+',this)">';
				s += '<label for="cmn-toggle-'+data[0]+'"></label>';
				s += '</div>'; 
				$('td', row).eq(3).html(s);
			} else {
				s +='<div class="switch">';
				s +='<input id="cmn-toggle-'+data[0]+'" class="cmn-toggle cmn-toggle-round" type="checkbox" onClick="fn.app.slide.active('+data[0]+',this)">';
				s += ' <label for="cmn-toggle-'+data[0]+'"></label>';
				s += '</div>';
				$('td', row).eq(3).html(s);
			}
			s = '';
			s += fn.engine.datatable.button('btn-default','fa-pencil','fn.app.slide.change('+data[0]+')');
			$('td', row).eq(4).html(s);
		}
	});
