<?php
	...
	$blog = $dbc->GetRecord("why_qrs","*","id=".$_REQUEST['id']);
	// $photo = $blog['image'];
...
<div class="modal fade" id="dialog_edit" data-backdrop="static">
  	<div class="modal-dialog">
		<form id="form_edit" class="form-horizontal" role="form" onSubmit="fn.app.why_qrs.save_change();return false;">
...
$(function(){
	...
	$("#fPhotoEdit").change(function(){
		...
		jQuery.ajax({
			url: 'apps/why_qrs/xhr/action-upload-photo.php',
			data: data,
			cache: false,
			contentType: false,
			processData: false,
...
$(function(){
	$("#fFileEdit").change(function(){
		...
		jQuery.ajax({
			url: 'apps/why_qrs/xhr/action-upload-file.php',
			data: data,
			cache: false,
			contentType: false,
			processData: false,
