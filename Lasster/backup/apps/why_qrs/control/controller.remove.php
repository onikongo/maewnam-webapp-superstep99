<?php
	include_once "apps/why_qrs/view/dialog.remove.php";
?>
<script style="text/javascript">
	
	fn.app.why_qrs.remove = function(){
	...
	$("#why_qrs .itoolbar").append('<button id="btnRemove" type="button" class="btn btn-danger pull-right">Remove</button>');
	$("#btnRemove").click(function(){
		fn.app.why_qrs.remove();
	});
	...
	$("#dialog_remove .btnConfirm").click(function(){
		var item_selected = $("#tbl").data("selected");
		$.post('apps/why_qrs/xhr/action-remove.php',{items:item_selected},function(response){
			$("#tbl").data("selected",[]);
			$("#tbl").DataTable().draw();
			$('#dialog_remove').modal('hide');
		});
